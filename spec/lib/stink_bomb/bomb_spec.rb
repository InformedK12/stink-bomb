module StinkBomb
  ...
  describe Bomb, '#trigger' do
    it 'does nothing if tomorrow is given' do
      expect do
        Bomb.new(Date.today + 1).trigger
      end.not_to raise_error
    end

    it 'does nothing if now + epsilon is given' do
      expect do
        Bomb.new(Time.now.getlocal + 1).trigger
      end.not_to raise_error
    end
    ...
    it 'raises a custom message when specified' do
      ...
      expect do
        Bomb.new(yesterday, message: 'Smells like poo').trigger
